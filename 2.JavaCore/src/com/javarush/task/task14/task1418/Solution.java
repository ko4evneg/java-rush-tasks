...
2. Создай в классе Solution статические методы , printListValues(List<Number> list), processCastedObjects(List<Number> list).
    ...
    static void processCastedObjects(List<Number> list) {
        for (Number object : list) {
            ...
            if (object instanceof Float) {
                Double a = (double) object.floatValue();
                System.out.println("Is float value defined? " + !(a.isNaN()));
            } else if (object instanceof Double) {
                Float a = (float) object.doubleValue();
