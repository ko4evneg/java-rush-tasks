    ...
    public Sayable toSayable(final int i) {
        class DogPet extends SuperDog implements Sayable {
            ...
            public String say() {
                ...
                for (int j = 0; j < i; j++, woof.append("а"));
                if (i < 1)
                    return String.format("%2$s%s%2$s спит.", Dog.this.name, getSuperQuotes());
                else
                    return String.format("%2$s%s%2$s лает г%3$sв! %4$s", Dog.this.name, getSuperQuotes(), woof, formatter.format(new Date()));
            }
        }
        return new DogPet();
    }
